...
- **ห้ามแก้** CLAUDE.md, `.claude/rules/`, MCP config ระหว่าง session — cache หายถาวรตลอด session
- ตั้งค่าทุกอย่างก่อนเริ่ม session — แก้ระหว่างทาง = cache lost, ไม่กลับมา
- ถ้าต้องแก้จริงๆ → `/clear` แล้วเริ่ม session ใหม่
- CLAUDE.md structure: stable content (standards, rules) อยู่บน, dynamic content (test mapping) อยู่ล่าง → cache-aware boundaries
