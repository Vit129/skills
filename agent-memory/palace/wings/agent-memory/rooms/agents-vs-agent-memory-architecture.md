# .agents/ vs agent-memory/ vs agent-context-kit — Architecture Decision — 2026-04-21

## Context
User asked whether to merge `.agents/`, `agent-memory/`, and GitHub `ThanakritAof/agent-context-kit` or keep them separate.

## Systems Compared

| Aspect | `agent-memory/` | `.agents/AGENTS.md` | `agent-context-kit` |
|--------|---------------------|---------------------|---------------------|
...
- [2026-04-21] **Keep `.agents/` and `agent-memory/` separate** — different lifecycle, different purpose
  - `.agents/AGENTS.md` = entry point + routing (changes rarely, when skills added/removed)
  - `agent-memory/` = data layer (changes every session)
...
2. **Memory layer** (`agent-memory/`) — WHAT happened, WHAT works
