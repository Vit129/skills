...
- Removed `agent-memory/` from gitignore logic — agent-memory is now tracked in git (part of project context)
- Deleted gitignore append block from script, replaced with comment
- Cleared `ai-agent/.gitignore` which only had `agent-memory/`
