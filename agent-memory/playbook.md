...
|----|---------|-----|--------|---------|---------|-----------|
| CASE-001 | Saving memory after editing global skill/hook/rule but workspace is a project | Route to `.claude/agent-memory/` (not project). Ask: "global skill or project feature?" | tooling/memory-routing | Prevents lessons landing in wrong project | 1 | 1 |
...
| CASE-005 | User gives short command ("PBI-002") and agent auto-executes all phases without dialog | MUST still follow full dialog flow: Phase Announcement → Decision → wait → execute. Short ≠ skip. | tooling/aidlc | Added gotchas to SKILL.md; prevents future dialog bypass | 0 | 0 |
