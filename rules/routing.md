...
**Daily automation (opt-in, added 2026-08-11):** a Kouen Automation (`com.claude.daily-digest-scheduler`-adjacent but a separate mechanism — Kouen's own scheduler, not launchd) spawns a fresh `claude` session in `~/.claude` once every 1440 minutes (~daily, first fired ~09:36 local) running this exact flow. This is a deliberate, explicit override of the cost tradeoff above — the user asked for it after being told it costs a full spawned session per fire, not a cheap query. The confirm-first rule below is NOT weakened by this: the spawned session still presents guesses and waits for a human to confirm, it never auto-files. If the cost turns out not worth it in practice, disable via `mcp__kouen__kouenAutomationPause` (id `52875F60-E056-451D-8D95-D46AC60B1780`) rather than deleting — the on-request flow below still works standalone either way.
