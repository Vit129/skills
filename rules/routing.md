...
2. For each open task, try the live-session cwd cross-reference above for a candidate project guess (may well come up empty). Also run the content-based signal — write the task's raw title to a temp file with the Write tool first (never shell-interpolate a Kouen task title directly into a Bash command string; it's untrusted free text and could contain shell metacharacters), then `uv run ~/.claude/scripts/project_router.py --query-file <temp file path>` (Personal projects only by default; pass `--include-company` only if the user has said Company projects are in scope) — a second, weaker guess based on matching the task title against each project's README.md/PRODUCT.md content, not a project field on the task itself. Treat both as guesses, never as a resolved answer; if they disagree, say so rather than silently picking one.
3. Present the task's full detail (title, id, createdAt, cwd-based guess if any, content-based guess if any/confident) and ask the user to confirm/name the actual project — do not assume, even with a guess in hand, regardless of how many signals agree.
