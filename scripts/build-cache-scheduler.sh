...
# Weekly scan+delete of stale build/cache dirs (node_modules, .venv, .build,
# __pycache__, dist, build, egg-info, DerivedData, etc. — see
...
#
# Scans ~/Git/Personal, ~/.claude, and ~/.kiro (extended 2026-08-21) --
# clean-build-cache.sh's SKIP_DIRS excludes the subtrees under the latter two
# that look like build cache by name but are actually installed software
# (Kiro editor extensions, plugin marketplace source, other jobs' tmp dirs).
...
ROOTS=("$HOME/Git/Personal" "$HOME/.claude" "$HOME/.kiro")
