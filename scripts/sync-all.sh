...
# ── Hermes rules sync (added for four-agent parity) ──────────────────────────
# Note: HERMES.md is manually maintained — not auto-generated from CLAUDE.md.
# Only the rules/ files are synced to keep workflow + project-rules in lockstep.
sync_hermes_rules() {
  local src="$HOME/.claude/rules"
  local dst="$HOME/.hermes/rules"

  if [[ -n "$ONLY_FILTER" && "$ONLY_FILTER" != "rules" && "$ONLY_FILTER" != "hermes" ]]; then
    return
  fi

  echo -e "  ${BLUE}[H] Hermes rules${NC}  ~/.claude/rules/ → ~/.hermes/rules/"
  if [ "$DRY_RUN" -eq 1 ]; then
    echo -e "    ${DIM}would copy workflow.md + project-rules.md → $dst/${NC}"
    return
  fi

  mkdir -p "$dst"
  cp "$src/workflow.md"       "$dst/workflow.md"
  cp "$src/project-rules.md"  "$dst/project-rules.md"
  # agent-core.md and skill-map.md are NOT overwritten —
  # ~/.hermes/rules/skill-map.md is Hermes-specific and must not be clobbered.
  echo -e "    ${GREEN}✅ workflow.md + project-rules.md synced to $dst/${NC}"
  echo ""
}
...
# ── Hermes skills sync ────────────────────────────────────────────────────────
# Converts Claude skills to Hermes SKILL.md format (adds YAML frontmatter).
# Skips protected skills (portfolio-holdings, investment-research, passive-income)
# if they are newer than the Claude source.
sync_hermes_skills() {
  if [[ -n "$ONLY_FILTER" && "$ONLY_FILTER" != "skills" && "$ONLY_FILTER" != "hermes" ]]; then
    return
  fi

  echo -e "  ${BLUE}[H2] Hermes skills${NC}  ~/.claude/skills/ → ~/.hermes/skills/"

  local script="$HOME/.claude/scripts/sync-skills-to-hermes.sh"
  if [[ ! -f "$script" ]]; then
    echo -e "    ${YELLOW}⚠️  sync-skills-to-hermes.sh not found, skipping${NC}"
    return
  fi
