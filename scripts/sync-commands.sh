#!/bin/bash
# sync-commands.sh — Sync slash commands from ~/.claude/commands/ to all AI agent targets
#
...
CONFIG_FILE="$SCRIPT_DIR/sync-commands.config.json"
COMMANDS_SOURCE="$HOME/.claude/commands"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      echo "Usage: bash sync-commands.sh [--dry-run] [--target <name>] [--list]"
      echo ""
      echo "Options:"
  ...
  cat > "$CONFIG_FILE" << 'EOF'
{
  ...
  "targets": {
    "codex": {
      "enabled": true,
      "path": "~/.codex/commands",
      "description": "OpenAI Codex CLI — mirror slash commands",
      "mode": "mirror",
      "exclude": ["*.DS_Store"]
    },
    "gemini": {
      ...
      "description": "Google Gemini CLI — mirror slash commands",
      "mode": "mirror",
      "exclude": ["*.DS_Store"]
    }
  },
...
if [ ! -d "$COMMANDS_SOURCE" ]; then
  echo -e "${RED}❌ Commands source not found: $COMMANDS_SOURCE${NC}"
  exit 1
fi
...
if [ "$LIST_MODE" -eq 1 ]; then
  echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
  echo -e "${CYAN}📋 Sync Commands Configuration${NC}"
  echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
  echo ""
  echo -e "  ${BLUE}Source:${NC} $COMMANDS_SOURCE"
  echo ""
...
# ── Header ──────────────────────────────────────────────────
echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${CYAN}🔄 Sync Commands — ~/.claude/commands/ → Multi-Agent${NC}"
echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo ""
echo -e "  ${BLUE}Source:${NC} $COMMANDS_SOURCE"
...
sync_mirror() {
  ...
  if [ "$DRY_RUN" -eq 1 ]; then
    local file_count
    file_count=$(find "$COMMANDS_SOURCE" -name "*.md" -type f | wc -l | tr -d ' ')
    echo "     Would rsync: $COMMANDS_SOURCE/ → $target_path/"
    echo "     Command files: $file_count"
    return
  fi

  mkdir -p "$target_path"
  rsync -av --delete --exclude="*.DS_Store" \
    "$COMMANDS_SOURCE/" "$target_path/" > /dev/null 2>&1

  local file_count
  file_count=$(find "$target_path" -name "*.md" -type f | wc -l | tr -d ' ')
  echo -e "     ${GREEN}✅ Synced: $file_count command files${NC}"
}
