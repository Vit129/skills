#!/bin/bash
# sync-skills.sh — Sync AI skills from ~/.claude/skills/ to all AI agent targets
#
# Source of truth: ~/.claude/skills/ (configured in sync-skills.config.json)
# Targets:
#   - ~/.codex/skills/      (OpenAI Codex — mirror mode)
#   - ~/.gemini/skills/     (Google Gemini CLI — mirror mode)
#
# Note: Kiro IDE uses ~/.kiro/skills/ directly — NOT synced from here.
#       To sync from Kiro → Claude: bash ~/.kiro/scripts/sync-skills-to-claude.sh
#
# Usage:
...
#   bash ~/.claude/scripts/sync-skills.sh --target codex
#   bash ~/.claude/scripts/sync-skills.sh --target gemini
#   bash ~/.claude/scripts/sync-skills.sh --list
#
...
CONFIG_FILE="$SCRIPT_DIR/sync-skills.config.json"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      echo "Usage: bash sync-skills.sh [--dry-run] [--target <name>] [--list]"
      echo ""
      echo "Options:"
...
SKILLS_SOURCE=$(python3 -c "
import json
with open('$CONFIG_FILE') as f:
    cfg = json.load(f)
print(cfg.get('source', 'ai-agent/skills/ai-dlc'))
")
SKILLS_SOURCE="$(resolve_source_path "$SKILLS_SOURCE")"

# ── Validate source ─────────────────────────────────────────
if [ ! -d "$SKILLS_SOURCE" ]; then
  echo -e "${RED}❌ Skills source not found: $SKILLS_SOURCE${NC}"
...
if [ "$LIST_MODE" -eq 1 ]; then
  echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
  echo -e "${CYAN}📋 Sync Skills Configuration${NC}"
  echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
  echo ""
  echo -e "  ${BLUE}Source:${NC} $SKILLS_SOURCE"
  echo ""
...
# ── Header ──────────────────────────────────────────────────
echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${CYAN}🔄 Sync Skills — Single Source → Multi-Agent${NC}"
echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo ""
echo -e "  ${BLUE}Source:${NC} $SKILLS_SOURCE"
...
")
  ...
  if [ "$DRY_RUN" -eq 1 ]; then
    echo "     Would rsync: $SKILLS_SOURCE/ → $target_path/"
    echo "     Excludes: $exclude_args"
    local file_count
    file_count=$(python3 -c "
import fnmatch, json
from pathlib import Path
source = Path('$SKILLS_SOURCE')
with open('$CONFIG_FILE') as f:
    cfg = json.load(f)
excludes = cfg['targets']['$target_name'].get('exclude', [])
count = 0
...
")
  ...
  rsync -av --delete --delete-excluded \
    $exclude_args \
    "$SKILLS_SOURCE/" "$target_path/" > /dev/null 2>&1
...
sync_steering() {
  ...
  while IFS= read -r skill_file; do
    ...
    local rel_path="${skill_file#$SKILLS_SOURCE/}"
...
<!-- Auto-generated from: ai-agent/skills/$rel_path -->
...
<!-- Auto-generated by sync-skills.sh from ai-agent/skills/ -->
<!-- Last synced: $(date +%Y-%m-%d\ %H:%M:%S) -->
...
echo ""
echo "  Run again:     bash ~/.claude/scripts/sync-skills.sh"
echo "  Preview only:  bash ~/.claude/scripts/sync-skills.sh --dry-run"
echo "  Config:        ~/.claude/scripts/sync-skills.config.json"
echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
