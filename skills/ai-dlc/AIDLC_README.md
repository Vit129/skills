...
> **Exception:** Postman migration → ใช้ `postman-to-playwright/postman/SKILL.md` โดยตรง ไม่ต้องผ่าน AIDLC
...
### `core/brainstorming/` — Multi-Role Brainstorming (Party Mode)
**Triggers:** "brainstorm", "คิดก่อน", "ยังไม่แน่ใจ", "explore", "ลองคิด", "ช่วยคิด", "อยากทำ", "มีไอเดีย", "ก่อนเริ่ม", "party mode", "ให้ทุก role ช่วยคิด"

> 💡 ใช้ **ก่อน** `core/aidlc/` เมื่อ idea ยังไม่ชัด — output จาก skill นี้คือ input ของ DECISIONS phase
> ทุก role (PO, Dev, QA) ร่วมกันถามและ challenge ใน session เดียว (BMAD Party Mode style)

| Reference | ใช้เมื่อ |
|-----------|---------|
| `po-lens.md` | คำถามจากมุม PO — user value, scope, success criteria |
| `dev-lens.md` | คำถามจากมุม Dev — feasibility, risks, architecture |
| `qa-lens.md` | คำถามจากมุม QA — testability, edge cases, AC draft |
| `output-template.md` | Format ของ output ก่อน handoff ไป AIDLC |

### `core/subagent-driven/` — Subagent-Driven Development
**Triggers:** "spawn subagent", "ใช้ subagent", "parallel tasks", "dispatch agent", "2-stage review", "subagent-driven", "auto-dispatch"

> ⚡ ใช้ **ระหว่าง Phase 3.1** เมื่อมี 3+ tasks ที่ independent — auto-dispatch หรือ manual dispatch
> Supports: Kiro (invokeSubAgent), Claude Code (Task), Gemini CLI (subprocess)
...
### `core/curator/` — Autonomous Skill & Knowledge Maintenance
**Triggers:** "curate skills", "grade knowledge", "prune dead skills", "consolidate skills", "curator run", "skill health check", "ดูแล skill", "จัดระเบียบ knowledge"

> 🧹 ใช้เมื่อต้องการ grade, consolidate, หรือ prune skills/knowledge อัตโนมัติ
> รันได้ manual, hook-driven, หรือเป็น subagent — ทุกอย่างเป็น markdown ไม่มี DB

| Reference | ใช้เมื่อ |
|-----------|---------|
| `grading-rubric.md` | Score skills/knowledge ตาม usage + recency + quality |
| `consolidation-rules.md` | Merge duplicates / overlapping patterns |
| `pruning-rules.md` | Archive unused items (never delete) |

### `core/scheduler/` — Recurring Task Automation
**Triggers:** "schedule task", "daily update", "cron", "ตั้งเวลา", "nightly test", "weekly report", "scheduled curator"

> ⏰ Cron-like scheduling สำหรับ dev/QA recurring tasks
> Jobs defined in `agent-memory/schedule.md` — trigger via hook, system cron, CI, or manual

Built-in templates: Daily Dev Update, Daily QA Update, Weekly Curator, Nightly Regression
...
| Reference | ใช้เมื่อ |
|-----------|---------|
| `workflow.md` | Phase routing, anti-shortcut rules, quick commands |
| `decision.md` | DECISIONS file structure, mandatory approval |
...
| `related-skills.md` | Skill routing guide |
...
**Triggers:** "add logging", "set up monitoring", "track errors", "slow response", "set up alerts"

| Reference | ใช้เมื่อ |
|-----------|---------|
| `logging.md` | Structured logs, log levels, correlation ID |
| `error-tracking.md` | Capture, group, alert on exceptions |
| `performance.md` | Latency, bottlenecks, SLO/SLA |
| `alerts.md` | Thresholds, notification routing |
| `tracing.md` | Distributed tracing (2+ services) |
| `quick-guide.md` | Where to start, observability overview |

### `core/storage/` — Knowledge Persistence
**Triggers:** "save knowledge", "update knowledge base", "index automation patterns", "save business rules"

| Reference | ใช้เมื่อ |
|-----------|---------|
| `automation-save.md` | Save API/Web UI/Mobile patterns |
| `business-save.md` | Save business rules, UI actions |
| `scenario-save.md` | Save test scenario PBIs |
| `buffer-update.md` | Update Knowledge Buffer |
| `data-backup.md` | Data safety standards |
...
| `pull-request.md` | PR creation, work item linking |
| `azure-sync.md` | Auto-create work items via MCP |
...
## po/ — Product & Architecture

### `po/architect/` — System Architecture
...
> Note: Step-Back Prompting อยู่ใน `core/analysis-skills/references/context.md`
