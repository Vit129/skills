...
### `core/interview-me/` — Interview Me (Requirements Extraction)
**Triggers:** "interview me", "ถามทีละข้อ", "ถามจนชัด", "grill me", "underspecified", "ไม่ชัด", "สัมภาษณ์"

> 🎯 ใช้ **BEFORE Phase 0** — เมื่อ requirement ไม่ชัด ถามทีละข้อจนถึง 95% confidence
> ต่างจาก brainstorming: interview-me = 1-on-1 (user + agent), brainstorming = multi-role (PO/Dev/QA subagents)

| Reference | ใช้เมื่อ |
|-----------|---------|
| (self-contained) | ONE question at a time → track confidence → summarize when ≥95% |
...
| Reference | เนื้อหา |
|-----------|---------|
| `pw-coding-standards.md` | AI governance, strategy, restrictions |
...
| Reference | เนื้อหา |
|-----------|---------|
| `rf-coding-standards.md` | AI governance, naming, locator priority, tags |
...
| Reference | เนื้อหา |
|-----------|---------|
| `ts-standards.md` | Title format, priority, categories, language policy |
...
| `ts-design.md` | Generate scenarios (CoT + 2026 standards) |
...
### `qa/verification-loop/` — Verification Loop (Pre-Commit Gate)
**Triggers:** "verify", "quality gate", "pre-commit check", "build+test+lint", "ready to commit?"

> ✅ ใช้หลัง implement ทุก task ก่อน commit — build → lint → test → coverage → security
> Mandatory ใน Phase 3 ก่อน mark task done

| Reference | ใช้เมื่อ |
|-----------|---------|
| (self-contained) | 5-step verification + report format + failure handling |

### `qa/eval-harness/` — Eval Harness (Skill Quality Measurement)
**Triggers:** "eval", "measure skill quality", "consistency check", "pass@k", "skill reliability"

> 📊 วัด skill quality ด้วย structured eval runs — pass@k, pass^k, checkpoint eval
> ใช้หลังสร้าง/update skill หรือเมื่อ skill ถูก flag

| Reference | ใช้เมื่อ |
|-----------|---------|
| (self-contained) | 3 eval types + grading + scoring + report format |
...
| `csharp-dotnet.md` | ASP.NET Core, EF Core, Minimal APIs, Clean Architecture |
| `cpp.md` | Modern C++ (17/20/23), CMake, RAII, GoogleTest |
...
| `azure-devops-sync.md` | Auto-create work items via MCP |
