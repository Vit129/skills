...
|-----------|------|---------|
| Source code access | Codebase | Read/trace the fail path |
| Test runner (Playwright/Jest/etc.) | Shell tool | Reproduce reliably |
| `knowledge/lessons/` | Lessons learnt | Check if this bug class was seen before |
| Git history (`git log`, `git blame`) | Shell tool | Find when regression was introduced |
| Browser DevTools / `playwright-cli` | Debug tool | Inspect live state for web bugs |

## Human-in-the-Loop Points

| Step | Approval Type | When |
|------|--------------|------|
| After Step 1 (Reproduce) | Checkbox confirm | User confirms repro is reliable before proceeding |
| After Step 3 (Hypothesis list) | Single select | Present 3-5 ranked hypotheses — user picks which to test first |
| Before applying fix | Open field | Present fix + rationale — user approves or suggests alternative |
| After fix validated | Checkbox | User confirms fix is correct, not just symptom-hiding |

**Rule:** At hypothesis stage, always present multiple ranked options — never commit to a single theory without user input.

## Verification

After debug session completes:

- [ ] Reliable repro existed before fix was attempted
- [ ] Fail path was traced (not guessed)
- [ ] At least one hypothesis was disproved before committing to the fix
- [ ] Breadcrumb ledger maintained (every experiment logged)
- [ ] Fix addresses root cause, not symptom
- [ ] Regression test written that fails without fix, passes with it
- [ ] `knowledge/lessons/` checked for prior art

## Self-Learning

After user approves the fix:

1. **Record pattern:** Save bug class + root cause + fix approach to `knowledge/lessons/{platform}/{bug-class}.md`
2. **Update playbook:** If this is a new failure pattern, add entry to `agent-memory/playbook.md`
3. **Confidence:** Mark as `confidence: 1.0` (user-validated fix) vs `confidence: 0.7` (auto-healed)
4. **Cross-reference:** If 3+ bugs share the same root cause pattern → create `knowledge/lessons/{platform}/{pattern}-pattern.md`

---

## Bug Life Cycle Integration

This skill handles **REPRODUCE → FIX**. After fix is validated, hand off to complete the lifecycle:

```text
find-mismatch (DETECT+CLASSIFY)
      ↓ finding confirmed
debug-mantra ← YOU ARE HERE (REPRODUCE+FIX)
      ↓ fix landed + validated
playwright-testing / robotframework-testing (GUARD — write regression test)
      ↓ regression test passes
post-mortem (CLOSED — document root cause + "why it slipped through")
      ↓ lesson captured
agent-memory (playbook → knowledge/bug/)
```

**After fix is validated, ALWAYS:**
