...
Grill with codebase awareness — cross-ref code, sharpen language, update GLOSSARY.md + ADRs inline.

## Step 1 — Load Context First
```
1. Read GLOSSARY.md (project root) + scan codebase structure
2. Read docs/adr/ for existing decisions
3. Report: "CONTEXT: 12 terms, 3 ADRs, modules: src/ordering/ src/billing/"
```
If no GLOSSARY.md → create when first term is resolved (never create empty).
...
```
GLOSSARY.md defines "Order" as confirmed purchase
User says "draft order" → in this codebase that's "Cart"
→ which term to use?
```

## Step 5 — Update Docs Inline (never batch)
Follow `references/domain-modeling.md` for all GLOSSARY.md and ADR rules (glossary only, ADR criteria, file structure).

## Summary Format
```markdown
## Interview-Doc Summary
**Plan:** [1-2 sentences]
**GLOSSARY.md updates:** Added/Updated/Flagged [terms]
