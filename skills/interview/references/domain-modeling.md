...
Active discipline for building and sharpening a project's domain model. Use when updating LANGUAGE.md or ADRs — not just reading them.

## LANGUAGE.md — strict glossary only

LANGUAGE.md is a **glossary and nothing else**. No implementation details, no specs, no scratch notes, no decision rationale. If it wouldn't appear in a conversation between a domain expert and a developer explaining "what things are called", it doesn't belong here.

File structure:
```
/
├── LANGUAGE.md          ← single context (most repos)
└── docs/adr/
...
When the user uses a term that conflicts with LANGUAGE.md, call it out immediately.
```
...
### Update LANGUAGE.md inline — never batch
When a term is resolved, write it to LANGUAGE.md immediately. Don't accumulate.
