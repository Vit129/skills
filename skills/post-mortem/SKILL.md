...
For the up-the-org version of this same content, hand the finished post-mortem to [`management-talk`](../../review/management-talk/SKILL.md). They compose: post-mortem owns the engineering truth, management-talk reframes it for leadership.
...
|-----------|------|---------|
| Debug session breadcrumb ledger | From `debug-mantra` | Raw material for "How it was found" section |
| PR / commit link | Git | Evidence for "Fix" section |
| Test results (before/after) | Test runner | Evidence for "Validation" section |
| JIRA API (MCP `azure-devops` tools) | Integration | Post comment to ticket |
| `review/management-talk/` | Companion skill | Leadership-flavored version handoff |
...
| After draft produced | Open field | User reviews accuracy of root cause + fix sections |
| Before posting to JIRA | Single select | User picks: "post it" / "edit first" / "print only" |
| After management-talk handoff offer | Single select | User picks: "yes, reframe for leadership" / "no, done" |

**Rule:** Always show the exact payload before posting. Never post without explicit "go ahead."

## Verification

After post-mortem is drafted:
...
- [ ] Validation states coverage honestly (what was/wasn't tested)
- [ ] Action items have owners + tracking artifacts
- [ ] No hedging language ("we believe", "appears to")

## Self-Learning

After user approves and posts the post-mortem:

1. **Record bug class:** Save root cause category + "why it slipped through" to `knowledge/lessons/{platform}/{bug-class}.md`
2. **Update playbook:** Add entry to `agent-memory/playbook.md` with the fix pattern
3. **Cross-reference:** If this bug class matches an existing lesson → increment `applied_count`
4. **CI gap tracking:** If "why it slipped through" = CI gap → flag for follow-up in next sprint planning

---

## Bug Life Cycle Integration

This skill handles **CLOSED** — the final documentation state. It requires all prior states to be complete:
...
debug-mantra (REPRODUCE+FIX) — root cause identified, fix validated
      ↓
playwright-testing (GUARD) — regression test written and passing
      ↓
post-mortem ← YOU ARE HERE (CLOSED) — document everything
      ↓
agent-memory (knowledge/bug/) — lesson persisted for future
```

**Pre-conditions (refuse to draft without):**
- REPRODUCE: reliable repro exists ✓
- FIX: fix is identified + validated ✓
- GUARD: regression test exists ✓ (mention in Validation section)
