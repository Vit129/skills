...
## Process
1. Locate MD file: `testScenarioPbi{pbi_id}.md` (e.g. `testScenarioPbi275957.md`)
   - Folder: `pbi-{pbi_id}-{feature-kebab}/` (e.g. `pbi-275957-document-status-reason/`)
   - MD header must have `**PBI-ID:** {pbi_id}` (for human reference — ID parsed from filename by script)
...
- **Checklist C:** Test Scenario Row — Col 1="Test Case", State="To Do", Test result="Not start", Actual Result empty, single backslash paths
