...
credit: Based on 9arm-skills (https://github.com/thananon/9arm-skills) — productivity/management-talk
version: 1.0.0
...
|-----------|------|---------|
| Source content (JIRA ticket / pasted text / chat context) | Input | The engineering content to reframe |
| JIRA API (MCP `azure-devops` tools) | Integration | Fetch ticket details, post comments |
| `debugging/post-mortem/` | Companion skill | Source of engineering truth to reframe |
| Channel format knowledge | Inline | JIRA / Slack / standup / email / meeting shapes |

## Human-in-the-Loop Points

| Step | Approval Type | When |
|------|--------------|------|
| Channel confirmation | Single select | If channel unclear — ask: "JIRA, Slack, standup, or email?" |
| After draft produced | Open field | User reviews tone + accuracy before using |
| Before posting to JIRA | Checkbox | Show exact ADF payload — user says "post it" or edits |
| After 3rd revision | Open field | Ask what specific framing/audience assumption is wrong |

**Rule:** Always present the draft shaped for the channel. Never post to non-JIRA destinations — hand to user.

## Verification

After management-talk draft is produced:
...
After user approves and uses the draft:

1. **Record good example:** Save approved draft + channel type to `knowledge/lessons/communication/{channel}-example.md`
2. **Record rejections:** If user rewrites significantly → note what framing was wrong, update tone guidance
3. **Progressive update:** If a new channel shape emerges (e.g., Teams, Notion) → append to Channel Shapes section
4. **Pattern recognition:** If same source type keeps needing the same reframe → create shortcut template
