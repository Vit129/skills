...
Four specialist perspectives for reviewing code before merge. Each persona focuses on a single axis — use one directly, or fan-out all four for a comprehensive pre-merge gate.
...
- "Find hidden bugs" / "scan for mismatches" → bug-hunter
- "Pre-merge gate" / "ship check" → fan-out all four
...
> 📦 Full methodology: `engineering/find-mismatch/SKILL.md`
> This persona applies the find-mismatch checklist in a review context.
...
Every finding must progress through this lifecycle. A bug is not "fixed" until a regression test guards it.
...
- **False positive check** — before reporting, verify it's not intentional (check comments, tests, docs)
...
- [ ] At least one persona ran (code-reviewer, test-engineer, security-auditor, or bug-hunter)
...
- [ ] For pre-merge gate: all 4 personas ran and findings merged
- [ ] Bug-hunter findings have lifecycle state assigned
...
|-----------|------|---------|
| Code diff / PR (files changed) | Review target | Primary input for all 4 personas |
| Existing conventions (lint, style, patterns) | Standards | Evaluate code against project norms |
| Security checklist (OWASP, auth patterns) | Security baseline | Security auditor persona reference |
| Test suite (existing coverage) | Quality data | Test engineer persona gap analysis |
...
|------|--------------|------|
| After review report | Checkbox (acknowledge findings) | After persona produces findings |
| Before posting verdict (APPROVE/REQUEST CHANGES) | Single select | Before finalizing review decision |
| Fan-out scope selection | Checkbox (which personas to run) | When pre-merge gate requested |
...
1. **Record good example:** Save approved output to `knowledge/lessons/review/{pattern}.md`
