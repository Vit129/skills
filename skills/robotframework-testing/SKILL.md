...
> If any step is skipped without a documented skip condition, flag it in the session's own review — no automated hook tracks this.
...
| `knowledge/qa/` | Lessons learnt | Check before execute |
...
After user approves the output — create a draft in `drafts/` (trigger, fix, domain, outcome) per `agent-memory`'s Promotion Flow, then score it with the Save/Discard Gate (2/3 criteria: novel, likely to recur, non-trivial) at session end. Fails → discard, don't save.

1. **Record good example:** draft → `PLAYBOOK.md` → if the same case recurs (Applied >= 3), promote to `knowledge/qa/{pattern}.md`
2. **Record failures:** if output was rejected → note what went wrong for next time
3. **Progressive update:** if a new pattern proved effective → append to the relevant knowledge index
