...
|-----------|------|---------|
| Pre-launch checklist (this skill) | Process guide | Ensure all quality gates pass |
| Monitoring tools (Datadog/Sentry/CloudWatch) | Infrastructure | Observe production behavior post-deploy |
| Rollback procedures (feature flags, git revert) | Safety net | Enable quick recovery if issues arise |
| Environment config (staging → production) | Infrastructure | Verify correct env vars and secrets |
...
|------|--------------|------|
| After checklist review | Checkbox (all items pass) | Before initiating deployment |
| Before production deploy | Single select (deploy now / schedule / hold) | At production gate |
| Rollback decision | Single select (rollback / hold / investigate) | When metrics exceed thresholds |
...
1. **Record good example:** Save approved output to `knowledge/lessons/shipping/{pattern}.md`
