...
```text
Memory Palace  = what happened (tracked in-context, saved as Memory Block)
Knowledge      = what works (lessons + patterns, saved in Memory Block)
Session Save   = output updated Memory Block → user copies to Project Instructions
```

## How It Works

**No files.** Memory lives in two places:
1. **In-context** — agent tracks state during the current conversation
...
```markdown
<!-- MEMORY BLOCK v1 | Updated: YYYY-MM-DD -->
...
## Knowledge
### Lessons
| ID | Domain | Summary | Applied |
|----|--------|---------|---------|
| L001 | {domain} | {lesson} | {count} |
...
3. Score-based routing: match current prompt against Lessons/Patterns (top 2)
4. Nudge check (max 2):
...
2. Update in-context state:
   ...
   - Add lessons/patterns to Knowledge
   - Increment sessions_since_consolidation
3. Output the complete updated Memory Block in a code block
4. Tell user: "Copy this Memory Block and paste it into [platform instructions]"
...
- If dirty=false at end → no save needed, tell user
...
When chat memory becomes insufficient (complex project, many topics):
- Move to IDE (Claude Code / Kiro) with full `agent-memory` skill
- Export Memory Block → use as seed for `palace/state.md` + `knowledge/index.md`
