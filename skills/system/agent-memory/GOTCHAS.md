...
### 28. First session crashes — no agent-memory/ exists
...
**Fix:** Step 0 Bootstrap in session.md: check if `agent-memory/` exists. If not → auto-create full directory tree with empty templates. Log "🏛️ Memory Palace initialized". Skip to Execute.
