...
If agent-memory/ exists → proceed to Step 1
```

### Step 1: Load Palace State
```
Read agent-memory/palace/state.md
Read agent-memory/palace/user-profile.md (if exists)
Extract:
...
```
Read agent-memory/palace/wings/knowledge-evolution/hall.md
...
```
During work:
  - Use templates from agent-memory/knowledge/ (global or per-project)
...
```
3a. Update agent-memory/palace/wings/knowledge-evolution/rooms/:
...
agent-memory/knowledge/index.json = source of truth (survives sessions)

For each score change in template-health.md:
  1. Read {project_root}/agent-memory/knowledge/index.json
...
For each lesson change in lesson-effectiveness.md:
  1. Read agent-memory/knowledge/lessons/{domain}/*LessonsIndex.json
...
"✅ Session saved:
 ...
 - Synced: agent-memory/knowledge/index.json ✓
