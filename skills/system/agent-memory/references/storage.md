...
**Rules:**
- Top-level agent-memory/palace/ contains ONLY: state.md, tunnels.md, user-profile.md, search-index.md, keyword-index.json, date-index.json, wings/, archive/
...
### Storage
```
agent-memory/palace/user-profile.md   ← single file, ≤80 lines
```
...
```json
{
  ...
  "terms": {
    "{keyword}": {
      ...
      "postings": [
        {
          "path": "wings/agent-memory/rooms/auth.md",
          "date": "2026-04-20",
          "wing": "agent-memory",
          "tf": 5,
...
Wing filter ("auth in agent-memory"):
  1. Lookup terms["auth"] → postings
  2. Filter: posting.wing == "agent-memory"
  ...
  "by_date": [
    {
      "date": "2026-04-21",
      "wing": "agent-memory",
      "path": "wings/agent-memory/rooms/search-scaling-research.md",
