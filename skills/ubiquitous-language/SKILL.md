...
description: >
  Post-hoc extraction of domain terms from the current conversation into CONTEXT.md.
...
Scan the current conversation for domain terms, resolve ambiguities, and write canonical definitions to CONTEXT.md.
...
Follow the `domain-modeling` reference for CONTEXT.md rules (glossary only, no impl details).

If CONTEXT.md already exists → merge new terms in, update definitions that have evolved, re-flag new ambiguities.
If CONTEXT.md doesn't exist → create it now with the resolved terms.
...
- **Glossary only.** CONTEXT.md gets term definitions — no implementation details, no specs, no decision notes.
...
When invoked again in the same session, read the existing CONTEXT.md first, incorporate new terms from the subsequent discussion, update evolved definitions, and re-flag any new ambiguities.
